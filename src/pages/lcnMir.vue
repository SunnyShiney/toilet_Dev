  ...
  <div>
    <div class="mapDiv">
      ...
      >
        ...
        ></tdt-polygon>
        <!-- <tdt-marker
          :position="toilet_one.position"
          @click="toiletDetail"
          :icon="toilet_one.icon"
        >
        </tdt-marker>
        <tdt-marker
          :position="toilet_two.position"
          @click="toiletDetail"
          :icon="toilet_two.icon"
        >
        </tdt-marker> -->
      ...
      <tdt-marker
        ...
        @click="toiletDetail(marker.title)"
      ></tdt-marker>
      ...
      <tdt-control>
        ...
        clearable
        class="inline-input w-10"
        placeholder="请输入厕所名称"
        @select="handleSelect"
      />
        </tdt-control>
        <tdt-control position="bottomleft" :visible="state.visible">
          <el-card class="toiletDetailBox">
            <template #header>
              <div class="card-header">
                <span>金牛区{{location_name}}公厕</span>
              </div>
            </template>
            <div style="font-size: 0.2rem">
              <div class="occupancy flex justify-between items-center">
                <span>厕位占用情况</span>
                <el-button
                  class="detail-btn"
                  type="primary"
                  size="small"
                  @click="toTvMap(location_name)"
                  >查看详情</el-button
                >
              </div>

              <div class="flex justify-around occupancyBody">
                <span
                  >男卫生间总位：<span style="color: red">{{
                    man_total_pit
                  }}</span></span
                >
                <span
                  >男卫生间空位：<span style="color: rgb(26, 179, 95)">{{
                    man_unoccupied
                  }}</span></span
                >
              </div>
              <div class="flex justify-around occupancyBody">
                <span
                  >女卫生间总位：<span style="color: red">{{
                    woman_total_pit
                  }}</span></span
                >
                <span
                  >女卫生间空位：<span style="color: rgb(26, 179, 95)">{{
                    woman_unoccupied
                  }}</span></span
                >
              </div>
              <div class="flex justify-around occupancyBody">
                <span
                  >第三卫生间总位：<span style="color: red">{{
                    disan_total_pit
                  }}</span></span
                >
                <span
                  >第三卫生间空位：<span style="color: rgb(26, 179, 95)">{{
                    disan_unoccupied
                  }}</span></span
                >
              </div>

              <div class="occupancy">今日客流</div>
              <div class="flex justify-between flow">
                <span></span>
                <span style="color: rgb(3, 168, 208)">男厕</span>
                <span style="color: hotpink">女厕</span>
                <span style="color: orange">第三卫生间</span>
              </div>
              <div class="flex justify-between flowDetail">
                <span></span>
                <span style="color: rgb(3, 168, 208)">{{
                  man_today_crowd
                }}</span>
                <span style="color: hotpink">{{ woman_today_crowd }}</span>
                <span style="color: orange">{{ disan_today_crowd }}</span>
              </div>

              <div class="occupancy">用水量/用电量/用纸量</div>
              <div class="flex justify-between flow">
                <span></span>
                <span style="margin-left: 4vw">今日</span>
                <span>本月</span>
              </div>
              <div
                v-for="(item, index) in flows"
                :key="index"
                class="flex justify-between flows"
              >
              ...
              <div class="occupancy">环境监测</div>
              <div class="flex justify-between toilet_gender">
                <span></span>
                <span style="color: rgb(3, 168, 208)">男厕</span>
                <span style="color: hotpink">女厕</span>
                <span style="color: orange">第三卫生间</span>
              </div>
              <div class="flex justify-between environment">
                <span>温度</span>
                <span style="color: rgb(3, 168, 208)">{{
                  temperature_value
                }}</span>
                ...
                <span style="color: orange">{{ temperature_value }}</span>
              </div>
              <div class="flex justify-between environment">
                <span>湿度</span>
                <span style="color: rgb(3, 168, 208)">{{ humdity_value }}</span>
                <span style="color: hotpink">{{ woman_humdity_value }}</span>
                <span style="color: orange">{{ humdity_value }}</span>
              </div>
              <div class="flex justify-between environment">
                <span>氨气</span>
                <span style="color: rgb(3, 168, 208)">{{ nh3_value }}</span>
                <span style="color: hotpink">{{ woman_nh3_value }}</span>
                <span style="color: orange">{{ nh3_value }}</span>
              </div>
              <div class="flex justify-between environment">
                <span>硫化氢</span>
                <span style="color: rgb(3, 168, 208)">{{ h2s_value }}</span>
                <span style="color: hotpink">{{ woman_h2s_value }}</span>
                <span style="color: orange">{{ h2s_value }}</span>
              </div>
              <div class="flex justify-between environment">
                <span>PM2.5</span>
                <span style="color: rgb(3, 168, 208)">{{PM25_value}}</span>
                <span style="color: hotpink">{{PM25_value}}</span>
                <span style="color: orange">{{PM25_value}}</span>
              </div>
                            <div class="flex justify-between environment">
                <span>PM10</span>
                <span style="color: rgb(3, 168, 208)">{{PM10_value}}</span>
                <span style="color: hotpink">{{PM10_value}}</span>
                <span style="color: orange">{{PM10_value}}</span>
              </div>
                            <div class="flex justify-between environment">
                <span>气压</span>
                <span style="color: rgb(3, 168, 208)">{{pressure_value}}</span>
                <span style="color: hotpink">{{pressure_value}}</span>
                <span style="color: orange">{{pressure_value}}</span>
              </div>
                            <div class="flex justify-between environment">
                <span>CO2</span>
                <span style="color: rgb(3, 168, 208)">{{CO2_value}}</span>
                <span style="color: hotpink">{{CO2_value}}</span>
                <span style="color: orange">{{CO2_value}}</span>
              </div>
                            <div class="flex justify-between environment">
                <span>TVOC</span>
                <span style="color: rgb(3, 168, 208)">{{tovc_value}}</span>
                <span style="color: hotpink">{{tovc_value}}</span>
                <span style="color: orange">{{tovc_value}}</span>
              </div>
                            <div class="flex justify-between environment">
                <span>水浸状态</span>
                <span style="color: rgb(3, 168, 208)">{{leakage_status}}</span>
                <span style="color: hotpink">{{leakage_status}}</span>
                <span style="color: orange">{{leakage_status}}</span>
              </div>
                            <div class="flex justify-between environment">
                <span>剩余纸量</span>
                <span style="color: rgb(3, 168, 208)">{{remain_paper_value}}</span>
                <span style="color: hotpink">{{remain_paper_value}}</span>
                <span style="color: orange">{{remain_paper_value}}</span>
              </div>
              <div class="occupancy">满意度评价</div>
              <div class="flex justify-between evaluation">
                <span style="color: rgb(3, 208, 88)">非常满意</span>
                <span style="color: rgb(143, 192, 51)">一般</span>
                <span style="color: rgb(208, 3, 3)">不满意</span>
              </div>
              <div class="flex justify-between evaluation">
                <span style="color: rgb(3, 208, 88)">{{level_nice}}%</span>
                <span style="color: rgb(143, 192, 51)">{{level_normal}}%</span>
                <span style="color: rgb(208, 3, 3)">{{level_bad}}%</span>
              </div>
            </div>
          </el-card>
        </tdt-control>
...
const leakage_status = ref()

const woman_total_pit = ref();
const man_total_pit = ref();
const disan_total_pit = ref();
const woman_unoccupied = ref();
const man_unoccupied = ref();
const disan_unoccupied = ref();
...
const loadAll = () => {
  return [
    { value: '枣子巷', position: "[104.04, 30.67]" },
    { value: '抚琴文化站',  position: "[104.03, 30.68]"},
  ]
}
...
function fetchData() {
    ...
    if (resp.status == 200) {
      ...
      if (resp.data.tvoc == "暂无数据") {
        ...
        axios({
    url: "/api/toilet-monitor/getShuiJin",
    data: {
      toilet_location: location_name,
    },
    method: "post",
    ...
    if (resp.status == 200) {
      ...
      leakage_status.value = resp.data.leakage_status;
  ...
  //获取空余总位数
  axios({
    url: "/api/toilet-flashlight/getUnoccupied",
    data: {
      toilet_location: location_name,
    },
    method: "post",
    ...
    if (resp.status == 200) {
      woman_unoccupied.value = resp.data["女卫生间"] ?? 0;
      man_unoccupied.value = resp.data["男卫生间"] ?? 0;
      disan_unoccupied.value = resp.data["第三卫生间"] ?? 0;
    }
  });

  //获取坑位总数
  axios({
    url: "/api/toilet-sensor/getPitCount",
    data: {
      toilet_location: location_name,
    },
    method: "post",
    ...
    if (resp.status == 200) {
      woman_total_pit.value = resp.data["女卫生间"] ?? 0;
      man_total_pit.value = resp.data["男卫生间"] ?? 0;
      disan_total_pit.value = resp.data["第三卫生间"] ?? 0;
    }
  });
...
occupancy.value = [
  {
    gender: "男厕",
    total: man_total_pit.value,
    empty: man_unoccupied.value,
  },
  {
    gender: "女厕",
    total: woman_total_pit.value,
    empty: woman_unoccupied.value,
  },
  {
    gender: "第三卫生间",
    total: disan_total_pit.value,
    empty: disan_unoccupied.value,
  },
];

const personFlow = ref([]);
personFlow.value = [
  {
    gender: "男厕",
    flow: 10,
...
const toilet_1_visible = ref(false);
//地图在页面中的视野大小以及位置
const state = reactive({
  //中心点的经纬度，第一个是经度，第二个是维度
  center: [104.02740358713781, 30.73735604287413],
...
const markers = [
  {
    title: "枣子巷",
    position: [104.04, 30.67],
  },
  { title: "抚琴文化站", position: [104.03, 30.68] },
];
...
function closeInfowindow() {
  state.currentMarker = {}; // 清空当前标记
}

function toTvMap(location) {
  // router.push("/tvMap");
  if (location === '【金牛区面街】轻松驿站•智慧厕所' || location === '【金牛区西青路】轻松驿站•智慧厕所'|| location === '【金牛凯德广场】轻松驿站•智慧厕所' || location === '【金牛公园】轻松驿站•智慧厕所' || location === '【金牛花圃北路】轻松驿站•智慧厕所') {
    router.push({ path: "/mobileTvMap", query: { param: location } });
  } else {
    router.push({ path: "/tvMap", query: { param: location } });
  }
}
// //枣子巷公共厕所 坐标来源于高德开放平台 坐标拾取器
// const toilet_two = reactive({
//   position: [104.04, 30.67],
//   visible: true,
//   icon: {
//     iconUrl: iconUrl,
...
// });
// //抚琴文化站公共厕所
// const toilet_one = reactive({
//   position: [104.03, 30.68],
//   visible: true,
//   icon: {
//     iconUrl: iconUrl,
//     iconSize: [35, 40],
...
function toiletDetail(location) {
  location_name = location
  fetchData();
  //   //   router.push("/tvMap");
  //   // toilet_1_visible.value = true
  //   // 获取具有类名为 'toiletDetailBox' 的第一个 el-card 元素
  //   var cardElement = document.querySelector(".toiletDetailBox");
  //   // 检查是否找到了匹配的元素
  //   if (cardElement) {
  //     // 更改 display 属性
  //     cardElement.style.display = "block"; // 将 'none' 替换为您想要的 display 属性值
  //   }
}
